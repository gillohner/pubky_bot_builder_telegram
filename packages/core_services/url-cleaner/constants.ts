...
const ALT_FRONTEND_MAPPING_SCHEMA: JSONSchema = {
    ...
    properties: {
        ...
        replacement: {
            ...
            description: "Replacement URL pattern. Use $1, $2, etc. for captured groups from the pattern.",
            minLength: 1,
        },
        pathPattern: {
            type: "string",
...
export function validateMapping(mapping: unknown, index: number): ValidationError[] {
    ...
    if (typeof m.replacement !== "string" || m.replacement.length === 0) {
        errors.push({ path: `${prefix}.replacement`, message: "Replacement is required and must be a non-empty string" });
    }
    ...
    if (m.pathPattern !== undefined) {
        if (typeof m.pathPattern !== "string") {
            ...
            try {
                ...
                errors.push({ path: `${prefix}.pathPattern`, message: `Invalid regex: ${(e as Error).message}` });
            }
        }
    }
...
export function validateConfig(config: unknown): ValidationResult {
    ...
    const boolFields = ["showOriginalUrl", "showCleanedUrl", "showRemovedParams", "silentIfUnchanged"];
    ...
    if (c.maxUrlsPerMessage !== undefined) {
        if (typeof c.maxUrlsPerMessage !== "number" || !Number.isInteger(c.maxUrlsPerMessage)) {
            ...
            errors.push({ path: "maxUrlsPerMessage", message: "maxUrlsPerMessage must be between 1 and 10" });
        }
    }
