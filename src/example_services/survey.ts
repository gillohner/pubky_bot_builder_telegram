...
interface TelegramMessagePhotoSize { file_id: string }
...
type BaseResp = { kind: "reply"; text: string; state?: unknown; options?: Record<string, unknown>; deleteTrigger?: boolean } |
  { kind: "photo"; photo: string; caption?: string; state?: unknown; deleteTrigger?: boolean } | { kind: "none" };

async function run() {
  const rawChunks: Uint8Array[] = [];
  for await (const c of (Deno.stdin as unknown as { readable: ReadableStream<Uint8Array> }).readable) rawChunks.push(c);
  const total = rawChunks.reduce((n, c) => n + c.length, 0);
  const buf = new Uint8Array(total); let o = 0; for (const c of rawChunks) { buf.set(c, o); o += c.length; }
    ...
    if (stage === 1) {
      ...
      if (animal.length < 3) {
        ...
        resp = {
          ...
          state: { op: "replace", value: { stage: 3, color: st.color, animal } },
          deleteTrigger: true,
        };
      }
    } else if (stage === 3) {
      ...
      if (!photoRef) {
        resp = {
          ...
          state: { op: "merge", value: { stage: 3, color: st.color, animal: st.animal } },
          deleteTrigger: true,
        };
      } else {
        resp = {
